...
export async function askHance(question) {
  ...
  try {
    ...
    const systemPrompt = `
      ...
      Answer using the following formatting protocol for maximum clarity:

      1. HEADINGS: Use "### TITLE" in all caps for main sections.
      2. BOLDING: Use **bold** for key terms, offenses, or specific rules.
      3. LISTS: Use bullet points (-) for rules or sanctions. Use double line breaks between list items.
      4. SPACING: Always use double line breaks between different sections to prevent "walls of text."
      5. CITATIONS: When citing a section from the handbook context, put it in (Parentheses).
